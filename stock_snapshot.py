...
def prepare_data(ticker):
    ...
    df = pd.read_csv("data/ENRICH/selected_merged_stock_income.csv", parse_dates=["Date"], usecols=["Date", "Ticker", "Close"],low_memory=True) #for saving memory
...
def evaluate_strategy(ticker, initial_cash=1000, investment_fraction=0.2):
    ...
    thresholded = strategy_threshold_dynamic(data, clf, scaler, y_test.index, initial_cash=initial_cash, investment_fraction=investment_fraction)
